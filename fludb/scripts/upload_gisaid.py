...
def update_database(db_path, fasta_file, metadata_file, sanitize=False):
    ...
    for record in SeqIO.parse(fasta_file, "fasta"):
        header = record.id
        parts = [p.strip().replace(" ", "_") for p in header.split("|")]
        if len(parts) != 4:
        ...
        try:
            ...
            ''', (isolate_id.replace(" ", "_"), sample_id, sequence, database_origin))
    ...
    for _, row in metadata_df.iterrows():
        seq_id = str(row['Isolate_Id']).strip().replace(" ", "_")
        isolate_name = str(row.get('Isolate_Name', '')).strip().replace(" ", "_")
