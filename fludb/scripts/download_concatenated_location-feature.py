"""
2025-05-13 CURRENTLY DOES NOT WORK!!
"""
...
# Function to concatenate genome segments into a single genome
def concatenate_genomes(df, segments):
    # Combine the specified segments into a single genome sequence
    df['genome'] = df[segments].apply(lambda row: ''.join(row.dropna().astype(str)), axis=1)
    return df
...
# Function to split location column into region, country, division, and location
def split_location(df, location_col="location"):
    ...
    df["location"] = df[location_col].apply(lambda x: extract_levels(x, 3))
    return df.drop(columns=[location_col])

# Main function to handle argument parsing and workflow execution
def main(db_path, fasta_file, metadata_file, headers, filters, segments, complete_genomes, header_delimiter, curate_locations, concatenate):
    # Fetch data from the database
    df = fetch_data_from_db(db_path, filters, segments, complete_genomes)
    
    # Concatenate segments into a single genome if specified
    if concatenate:
        df = concatenate_genomes(df, segments)

    # Curate locations if specified
    if curate_locations:
        df = split_location(df, location_col="location")
    
    # Generate the FASTA and metadata files
...
if __name__ == "__main__":
    ...
    parser.add_argument('--curate-locations', action='store_true', help="Split hierarchical location column into region, country, division, and location.")
    parser.add_argument('--concatenate', action='store_true', help="Concatenate genome segments into a single genome sequence.")
    ...
    main(args.db, args.fasta, args.metadata, args.headers, filters, segments, args.complete_genomes, args.header_delimiter, args.curate_locations, args.concatenate)
